...
writeToLog(\`\${words.join(' ')}\`);`);
  ...
  // ...
    ...
    testExportImportWorkaraoundPart1: () => {
        printBeforeAndAfter(`export const ICONS = {
  email: \`foo.png\`,
};`);
    },
    testExportImportWorkaraoundPart2: () => {
        printBeforeAndAfter(`import { ICONS } from './package';
...
`);
    ...
    testExportImportNamespaceWorkaround: () => {
        printBeforeAndAfter(`namespace Package {
  export function foo() {}
}
...
nameIWantForMyImports();`);
    ...
    testTypeScript_34x_highOrder: () => {
        printBeforeAndAfter(`// Higher order function type inference
...
const t2 = flipped(true, 0);  // [number, boolean]`);
    ...
    testTypeScript_34x_improvedReadonly: () => {
        printBeforeAndAfter(`// Improved support for read-only arrays and tuples
...
type T4 = ReadWrite<Required<T3>>;  // string[]`);
    ...
    testTypeScript_34x_constContext: () => {
        printBeforeAndAfter(`// Const contexts for literal expressions
...
let z = { text: "hello" } as const;  // Type { readonly text: "hello" }`);
    ...
    testTypeScript_34x_globalThis: () => {
        printBeforeAndAfter(`// \`globalThis\`
// Add globalThis
// @Filename: one.ts
var a = 1;
var b = 2;
